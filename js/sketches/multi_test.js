...
const backgroundSketch = ( p ) => {
	...
	p.setup = function() {
		p.createCanvas(document.getElementById('header-design').offsetWidth, document.getElementById('header-design').offsetHeight);
	};
		...
		if (first_pass===true) {
			...
			for (let h = 0; h < height; h += height_interval) {
				...
				for (let i = 0; i < (width + width_interval); i += width_interval) {
					...
					// p1.color('rgba(228,228,228,0.1)')
					p1.fill('rgba(228,228,228,0.0025)')
					p1.polygon(0, 0, radius, 6);
					// p1.fill('rgba(196,196,196,0.05)')
					// p1.polygon_noise(0, 0, radius, 6, 0.055);
					p1.pop();
				}
				// pass += width_interval/45
			}
			// pass += 0.01
			first_pass=false
		}
...
let backgroundp5 = new p5(backgroundSketchTwo, 'header-design');
